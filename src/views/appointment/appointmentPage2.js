...
import {updateStepIndex, completeLoadingTransition, clearVisitedSteps, submitAppointment} from '../../actions/creators/appointment';
...
class AppointmentPage extends React.PureComponent {
    ...
    componentDidUpdate(previousProps) {
        if (!previousProps.loading && this.props.loading)
            setTimeout(() => this.props.actions.setLoadingTransition(), Timers.TRANSITION);
    }

    onRequestStep(stepIndex) {
        if (!this.props.visitedSteps.includes(this.props.stepIndex))
            this.props.actions.addVisitedStep(this.props.stepIndex);
...
function mapStateToProps(state) {
    const {stepIndex, visitedSteps, submitted, loading, saving} = state.appointment;
    const {currentStep, steps} = state.navigation;
    const content = currentStep.getStepContent();
    console.log("CONTENT", content);
    const {institution, contact, schedule} = state;
    const appointment = {institution, contact, schedule};
    const errorSteps = Object.keys(state.errorInfo)
        .filter(stepKey => Object.values(state.errorInfo[stepKey])
    ...
    return {stepIndex, visitedSteps, errorSteps, submitted, saving, loading, appointment};
}
...
function mapDispatchToProps(dispatch) {
    const actions = {
        ...
        clearVisitedSteps,
        submitAppointment,
        setLoadingTransition: completeLoadingTransition,
